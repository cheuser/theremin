...
// then an interrupt is fired on the falling edge and the proximity sensor
// is triggered.
//****************************************************************************
...
void main(void)
{
    ...
    GPIO_setAsOutputPin(GPIO_PORT_P3, GPIO_PIN0); //Trigger to prox sensor
    GPIO_setAsInputPinWithPullUpResistor(GPIO_PORT_P5, GPIO_PIN0);  // Set detector input
    GPIO_setAsInputPinWithPullUpResistor(selectedPort, selectedPins); //echo from prox sensor
